...
import { signupInput } from "@sarthakpatwal/medium-common";
...
export const Auth = ({ type }: { type: "signup" | "signin" }) => {
  ...
  async function sendRequest() {
    try {
      ...
      const jwt1 =  (response.data);
      console.log(jwt1)

      localStorage.setItem("token", jwt1.jwt);
...
function Labelledinput({
  ...
  return (
    <div>
      <label className=" mt-2 block mb-2 text-sm font-medium  text-black">
        {label}
      </label>
      <input
        onChange={onChange}
